...
DEFCONFIG=hyper_defconfig
DEFCONFIG_S6EDGE=hyper_defconfig_edge
DEFCONFIG_S6FLAT=hyper_defconfig_flat
TMOBILE_DEFCONFIG=hyper_defconfig_tmobile

export K_VERSION="v0.001"
export K_NAME="Hyper-Kernel"
export REVISION="RC"
...
clear
echo "**********************************"
echo "Hyper-Kernel Build Script"
echo "**********************************"
echo ""
echo ""
...
if [ $prompt == "1" ]; then
    ...
    export KERNEL_VERSION="$K_NAME-Oreo-$K_VERSION"
    echo "Compiling FLAT ..."
    ZIP_NAME=$K_NAME-G92X-O-$K_VERSION.zip
    MAIN
elif [ $prompt == "4" ]; then
    MODEL=G920
    DEVICE=$S6DEVICE
    ...
    ZIP_NAME=$K_NAME-$MODELT-O-$K_VERSION.zip
    MAIN
elif [ $prompt == "5" ]; then
    MODEL=G925
    DEVICE=$S6DEVICE
    ...
    export KERNEL_VERSION="$K_NAME-Oreo-$K_VERSION"
    echo "Compiling FLAT ..."
    ZIP_NAME=$K_NAME-G92XT-O-$K_VERSION.zip
    MAIN
fi
